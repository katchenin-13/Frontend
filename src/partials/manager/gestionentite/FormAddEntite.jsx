...
const FormAddEntite = () => {
    ...
    const { entites, addEntite, typeEntite } = useEntiteActions();
    ...
    const removeDynamicField = (id) => {
     ...
     const handleSubmit = (e) => {
           ...
           const emptyFields = Object.entries(EntiteData).filter(([_, value]) => {
           ...
           // Vérifier si l'ID existe pour mettre à jour ou ajouter
            const entiteExists = entites.some(entite => entite.email === EntiteData.email);
                   if (entiteExists) {
                       toast.error("Ce entite existe déjà avec cet email.");
                       return;
                   }
           
                   // Ajouter le entite via le contexte
                   addEntite(EntiteData);// Ajoute le entite via le contexte
                //localStorage.setItem("entite", JSON.stringify(EntiteData));// ✅ Stocke correctement l'objet sélectionné
   
                   localStorage.setItem("entites", JSON.stringify(entites)); // Sauvegarde des entites dans le localStorage
                   toast.success("entite ajouté avec succès !");
