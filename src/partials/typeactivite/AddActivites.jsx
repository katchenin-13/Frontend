...
function AddActivites() {
    ...
    const next = () => {
        if (formNo === 1 && state.motif && state.date && state.beneficiaire !== "---Selectionner un beneficiaire---" && state.typeactivitesdes !== "---Selectionner un type d'activité---") {
            ...
            setFormNo(formNo + 1);
            setStatus("en_cours_programmation");
        } else if (formNo === 3) {
            if (state.responsableName && state.responsableEmail && state.responsableFonction) {
                setFormNo(formNo + 1);
                setStatus("terminer");
            } else {
                setStatus("en_cours_assignation");
            }
        } else {
                ...
                <div className="relative flex flex-col w-full h-full text-gray-700 bg-white shadow-md rounded-xl">
                    ...
                    <div className="card w-full rounded-md shadow-md bg-white p-5">
                        <div className="flex justify-center items-center">
                            {formArray.map((v, i) => (
                                <>
                                    <div className={`w-[35px] my-3 text-white rounded-full ${formNo - 1 >= i ? "bg-blue-500" : "bg-slate-400"} h-[35px] flex justify-center items-center`}>
                                        {v}
                                    </div>
                                    {i !== formArray.length - 1 && <div className={`w-[85px] h-[2px] ${formNo > i ? "bg-blue-500" : "bg-slate-400"}`}></div>}
                                </>
                            ))}
                        </div>
                            ...
                            <form onSubmit={handleSubmit} className="border-gray-500 text-white p-6 rounded-2xl w-full max-w-4xl space-y-4">
                                ...
                                {state.typeactivitesdes === "Audience" && (
                                    <div className="border-2 border-gray-500 p-4 rounded-lg">
                                        <h2 className="font-semibold text-black uppercase">Autre info</h2>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                                            <ConfigurableInput label="Nom du demandeur" name="demandeurNom" value={state.responsableName} onChange={handleChange} required />
                                            ...
                                            <ConfigurableInput label="Email du demandeur" name="demandeurEmail" value={state.responsableEmail} onChange={handleChange} required />
                                            <ConfigurableInput label="Date de rencontre souhaitée" name="date" type="date" value={state.date} onChange={handleChange} required />
                                ...
                                <div className="flex justify-end mt-6 space-x-4">
                                    ...
                                    <button onClick={next} className="px-3 py-2 text-white bg-blue-500 w-full">Suivant</button>
